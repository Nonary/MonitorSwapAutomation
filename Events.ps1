...
function IsPrimaryMonitorActive() {
    ...
    $primaryProfile = Get-PrimaryMonitorIds -filePath "primary.cfg"
    $currentProfile = Get-PrimaryMonitorIds -filePath $filePath

    return ($primaryProfile -eq $currentProfile)
}
