# Download CRU (Custom Resolution Utility) and set a serial number on your primary display and the dummy plug.
# This is done by clicking "edit" on the display dropdown.
# If a serial number exists, you do not need to change it. 
# This automation requires you to check the box that states "Include in Slot if Available"
# After doing so, make sure you can see the serial in the detailed resolutions section.
# If you do not see it, you will need to delete a detailed resolution (they can be added in the CTA-861 block instead)

$primarySerial = "LGTV"
$dummySerial = "DUMMY"
# How many seconds to wait after a stream is suspended/terminated before swapping back.
$gracePeroid = 30
$configDirectory = $env:TEMP
...
function DisableOtherDisplays() {
    & .\MultiMonitorTool.exe /SaveConfig "$configDirectory\monitorinfo.ini"
    Start-Sleep -Seconds 1
    $monitorConfigLines = (Get-Content -Path "$configDirectory\monitorinfo.ini" | Select-String "MonitorID=.*|SerialNumber=.*|Width.*|Height.*|DisplayFrequency.*")
...
function RestoreMonitorPositions() {
    ...
    & .\MultiMonitorTool.exe /LoadConfig "$configDirectory\monitor-config.cfg"
}

function CaptureMonitorPositions() {
    # Export the users monitor configuration so it can be restored later.
    & .\MultiMonitorTool.exe /SaveConfig "$configDirectory\monitor-config.cfg"
}

function SetPrimaryScreen() {
    Write-Host "Attempting to set primary screen"
    if (IsCurrentlyStreaming) {
        return "No Operation necessary because user is still streaming"
    }
